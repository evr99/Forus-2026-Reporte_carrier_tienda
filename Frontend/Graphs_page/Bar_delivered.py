...
subheader("COMPROMISO ENTREGA y FECHA ACTUAL pedidos ENTREGADOS", divider = "blue")
...
try:
    ...
    download_button = select_container_not_delivered.button("Descarga Excel", key = "download_bar_delivered")
    ...
    total_ahead_time = data[(data["SLA STATUS"] == "ADELANTADO")].shape[0]

    if total_past_time > 0:
        messages_container.write(f"Total de pedidos que se entregaron tarde {total_past_time}")
    if total_on_time > 0:
        messages_container.write(f"Total de pedidos que se entregaron a tiempo {total_on_time}")
    if total_ahead_time > 0:
        messages_container.write(f"Total de pedidos que se entregaron adelantados {total_ahead_time}")

    columns_to_eliminate = ["SLA STATUS", "ESTADO", "DIAS ENTREGA"]
    data.drop(columns = columns_to_eliminate, axis = 1, inplace = True)
    ...
    if download_button:
        download_path = Path.home() / "Downloads" / "Reporte_pedidos_entregados.xlsx"
        counter = 1
